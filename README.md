...
Copy your Firebase service account JSON into `config/private/serviceAccountKey.json`. Promptly expects this file to include a `webConfig` block so the browser SDK can initialize:
...
If you keep the file outside this repo, symlink it into `config/private/serviceAccountKey.json` so deployments can copy it alongside the app.
...
3. In **Project Settings → General → Your apps**, copy the Web SDK config (API key, auth domain, app ID, messaging sender ID) into the `webConfig` block inside the same JSON file.
...
FIREBASE_API_KEY=$(node -p "JSON.parse(require('fs').readFileSync('config/private/serviceAccountKey.json','utf8')).webConfig.apiKey")
